...
IDEAL_PARTNER_CURVE = {
    1: 0.3, 2: 0.5, 3: 0.7, 4: 0.85, 5: 0.95, 6: 1.0,
...
def project_collection(partner: PartnerFinancials, month: int) -> float:
    ...
    avg_monthly = sum(partner.cash_collected) / len(partner.cash_collected) if partner.cash_collected else 0
...
def compare_to_ideal_curve(partner: PartnerFinancials) -> dict:
    ...
    return {
        ...
        "status": "AHEAD" if performance_index >= 1.0 else "ON TRACK" if performance_index >= 0.7 else "BEHIND"
    }
...
def assess_risk(partner: PartnerFinancials) -> dict:
    ...
    return {
        ...
        "at_risk": at_risk,
        "alert": f"⚠️ AT RISK: Projected €{projected:,.0f} < Draw €{partner.monthly_draw:,.0f}" if at_risk else "✅ Healthy"
    }
